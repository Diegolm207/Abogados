...
-- Volcando datos para la tabla lmpdb.entities: ~3 rows (aproximadamente)
...
CREATE TABLE IF NOT EXISTS `projects` (
  ...
  CONSTRAINT `FKPRJEmployee` FOREIGN KEY (`PRJIdEmployee`) REFERENCES `employees` (`EMPId`) ON DELETE SET NULL ON UPDATE SET NULL,
  CONSTRAINT `FKPRJEntity` FOREIGN KEY (`PRJIdEntity`) REFERENCES `entities` (`ENTId`) ON DELETE SET NULL ON UPDATE SET NULL
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8mb4;

-- Volcando datos para la tabla lmpdb.projects: ~2 rows (aproximadamente)
...
-- Volcando datos para la tabla lmpdb.status: ~10 rows (aproximadamente)
DELETE FROM `status`;
...
CREATE TABLE IF NOT EXISTS `tasks` (
  ...
  CONSTRAINT `FKTSKEmployee` FOREIGN KEY (`TSKIdEmployee`) REFERENCES `employees` (`EMPId`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FKTSKEntity` FOREIGN KEY (`TSKIdEntity`) REFERENCES `entities` (`ENTId`),
  CONSTRAINT `FKTSKProject` FOREIGN KEY (`TSKIdProject`) REFERENCES `projects` (`PRJId`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8mb4;

-- Volcando datos para la tabla lmpdb.tasks: ~1 rows (aproximadamente)
DELETE FROM `tasks`;
...
-- Volcando datos para la tabla lmpdb.types: ~8 rows (aproximadamente)
DELETE FROM `types`;
...
INSERT INTO `users` (`USRId`, `USRLogin`, `USRPsw`, `USREmail`, `USRName`, `USRLastName`, `USRDateInsert`, `USRInsertBy`, `USRDateUpdate`, `USRUpdateBy`) VALUES
	(1, 'ampineda', 'Amp000888', 'ampineda@outlook.com', 'Alberto', 'Martínez', NULL, NULL, '2020-12-06 18:32:00', 'ampineda'),
