<?php
    include '../controller/cntEmployee.php';
...
<html>
    ...
    -->
<head>
    <title>ERP - Alberto Martínez - Clientes</title>  
    <meta charset="utf-8">
...
<body class="bgsilver">
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark">
        <!-- Brand -->
        <a class="navbar-brand" href="home.php">Plataforma de Gestión Jurídica - Abogado - 
            <?php 
                if (isset ($_REQUEST['empinsert'])){                
                    $_SESSION['mode'] = 'empinsert';
                    $mode='NUEVO';
                    echo 'Nuevo';
                }
                if (isset ($_REQUEST['empupdate'])){
                    $_SESSION['mode'] = 'empupdate';
                    $mode='EDITAR';
                    echo 'Editar';
                }                
                if (isset ($_REQUEST['empdelete'])){                
                    $_SESSION['mode'] = 'empdelete';
                    $mode='ELIMINAR';
                    echo 'Eliminar';
                }
    ...
    <form name=dataidu class="w3-container" action="../controller/cntEmployee.php" method="post">
        ...
        <div class="row">
            <div class="col-md-6">
                <table class="table table-striped" style="font-size: 75%">
                    <?php
                        echo '<tr><td>Acción:</td><td><input type="text" name="empidu" value="'. $mode.'" readonly></td><td></td></tr>';
                        /*Show form with data or empty*/
                        if (isset($_REQUEST['empid'])){ 
                            selectEmpById($_REQUEST['empid']);
                        }
                        else {
                            selectEmpById(-1);
                        }
